...
export default function EditPostButton ({post_user_id, post_id}: { post_user_id : number, post_id : number}) {
    let isHidden = true;
    ...
    return (
        <button onClick={()=> {
                router.push(`/feed/edit-post/?id=${post_id}`);}} id="post-edit-button" type="button" className= {`  md:hover:animate-bounce rounded-md text-md  inline-flex  p-1 absolute right-2 top-1 ${isHidden ? "hidden" : ""}`}>
